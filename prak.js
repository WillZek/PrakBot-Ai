...
mobile: MethodMobile, 
browser: opcion == '1' ? ['PrakBot-Ai', 'Edge', '20.0.04'] : methodCodeQR ? ['PrakBot-Ai', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "PrakJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './PrakJadiBot';
if (!existsSync(subBotDirectory)) {
console.log('🌠 PrakBot-Ai no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'PrakSession';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🌠 Ai Prak conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar PrakBot-Ai: `, error))
}
})();
...
const directories = ['./PrakSession/', './PrakJadiBot/']
